...
| "Use / register an existing project ID" | `project-add --project-id EXISTING_ID --name "Name"` |
...
- If the user has an existing project ID → save it with: `project-add --project-id PID --name "name"`
- If the user wants a new project → run: `create-project`
...
**First, run `config --json` to check if project_id is set. If not, ask the user whether to use an existing project ID or create a new project.**

```
1. config --json  →  check local state for active_project
   - If not set and user has an existing project ID → save with project-add
   - If not set and user wants a new project → run create-project
...
python3 {baseDir}/agent_skill.py chat --prompt "generate ocean waves" --prefer-models '{"VIDEO":["generate_video_kling_v2_6"]}' --json --download

# Combine image and video preferences
python3 {baseDir}/agent_skill.py chat --prompt "create content" --prefer-models '{"IMAGE":["generate_image_seedream_v4_5"],"VIDEO":["generate_video_kling_v2_6"]}' --json --download
```

Available models for `--prefer-models` change server-side. Do NOT trust a stale hardcoded table when precision matters.

If the user explicitly asks to choose models based on routing, read `references/model-routing.yaml` first, use `references/model-routing.md` as explanation when needed, and apply the highest-priority matching rule after verifying live availability with `query-mode`.

Always query the live account state first:

```bash
python3 {baseDir}/agent_skill.py query-mode
```

As of **2026-04-22**, this account returned these currently available aliases:

**IMAGE (verified via `query-mode`):**
...
**VIDEO (verified via `query-mode`):**

- `generate_video_kling_v2_6`
- `generate_video_kling_omni_v1`
- `generate_video_wan_v2_6`

If the requested alias is not present in `query-mode`, do not force it in `--prefer-models`. Either omit the preference or choose one of the currently listed aliases.
...
python3 {baseDir}/agent_skill.py chat --prompt "generate a video" --include-tools generate_video_kling_v2_6 --json --download
```
