...
> **参考教训**：6/11 A 任务选了「喷泉」「经纬」，但这两首 6/10 已创作过（只是流派不同），导致 6/11 实际只有 1 首新歌。

确定 3 首歌名后、进入 Phase 2 之前：
1. 从 Step 1-2 搜索结果中提取所有 7 天内已用的 **歌名**（标题中 `·` 或 `—` 前的部分，或笔记标题中 `🎵` 后的部分）
2. 将本批 3 个候选歌名逐一对比已用歌名列表
3. **⚠️ 重复判定**：歌名完全相同（忽略大小写/空格），或歌名高度相似（如「喷泉」vs「小喷泉」），都算重复
4. 重复的歌名必须**重新命名或换主题**，不得沿用
5. 在去重报告中列出 7 天已用歌名列表

### 任务头部输出（每次必须）

```
去重报告：7 天内已用流派 X 种 / 主题 Y 个；24h 内已用 X / Y；本次可用 X / Y
7 天已用歌名：[歌名1, 歌名2, ...]
